...
log_must ([[DD, "if=/dev/zero", "of="+mntpnt+"/"+TESTFILE, "bs=1M", "count=20"]])
...
if used < refquota:
    log_fail("ERROR : "+str(used)+" < "+str(refquota)+" subfs quotas are limited by refquota") 

log_mustnot ([[DD, "if=/dev/zero", "of="+mntpnt+"/"+TESTFILE+".2", "bs=1M","count=20"]])
...
refquota = int(refquota) / (1024 * 1024)
if used > quota:
    log_fail("ERROR : "+str(used)+" > "+str(quota)+" subfs quota are limited by quota") 
