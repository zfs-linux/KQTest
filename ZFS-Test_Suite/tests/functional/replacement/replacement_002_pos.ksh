...
#
# ident	"@(#)replacement_002_pos.ksh	1.3	08/02/27 SMI"
#
...
i=0
while [[ $i != 2 ]]; do
dd  if=/dev/zero  of=$TESTDIR/$TESTFILE1.$i  bs=1M count=100
...
dd  if=/dev/zero  of=$TESTDIR/$REPLACEFILE  bs=1M count=100
...
for type in "" "raidz" "raidz1"; do
	for op in "" "-f"; do
		...
		log_must $ZFS set mountpoint=$TESTDIR1 $TESTPOOL1/$TESTFS1
		$ZPOOL status 
		log_mustnot $ZPOOL attach "$opt" $TESTDIR/$TESTFILE1.1  $TESTDIR/$REPLACEFILE
		$ZPOOL status
		$ZPOOL iostat -v $TESTPOOL1 | grep "$TESTDIR/$REPLACEFILE"
		if [[ $? -eq 0 ]]; then
