...
main()
{
	...
	case 0:
		while (i > 0) {
			...
			int c_count = 0;
			if (rename("/tank/zfs/a/b/c", "/tank/zfs/1/2/3/c") == 0)
				c_count++;
			if (rename("/tank/zfs/1/2/3/c", "/tank/zfs/a/b/c") == 0)
				c_count++;
			if (c_count) {
			
				printf(" C Count : %d\n", c_count);
				(void) strcat(buf, "c_count: %d,");
				(void) strcat(buf, msg);
	...
	default:
		while (i > 0) {
			...
			int p_count = 0;
			if (rename("/tank/zfs/1", "/tank/zfs/a/b/c/d/e/1") == 0)
				p_count++;
			if (rename("/tank/zfs/a/b/c/d/e/1", "/tank/zfs/1") == 0)
				p_count++;
			if (p_count) {

				printf(" p Count : %d\n", p_count);
				(void) strncat(buf, "p_count: %d,", 256);
				(void) strncat(buf, msg, 256);
