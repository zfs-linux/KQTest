...
class test_grow_pool(unittest.TestCase):
	...
	(ret, stdout, stderr) = cmdQuery([path + "/grow_pool/setup.py", disk], env=newenv, cwd=path+"/grow_pool")
    ...
    def cleanup(self):
        (newenv, path) = lib.STFwrap.setupEnv()
        cmdQuery([path + "/grow_pool/cleanup.py"], env=newenv, cwd=path+"/grow_pool")
    ...
    def test_grow_pool_001_pos(self):
        ...
        disk = "/dev/" + disk
        lib.STFwrap.runScriptArgs(self, ["/grow_pool/grow_pool_001_pos.py", disk])
