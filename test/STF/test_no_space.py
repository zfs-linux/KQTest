...
class test_no_space(unittest.TestCase):
    def setUp(self):
        ...
        (ret, stdout, stderr) = cmdQuery([path + "/no_space/setup.py" , disk], env=newenv, cwd=path+"/no_space")
    ...
    def cleanup(self):
        (newenv, path) = lib.STFwrap.setupEnv()
        cmdQuery([path + "/no_space/cleanup.py"], env=newenv, cwd=path+"/no_space")

    def tearDown(self):
        (newenv, path) = lib.STFwrap.setupEnv()
        (ret, stdout, stderr) = cmdQuery([path + "/no_space/cleanup.py"], env=newenv, cwd=path+"/no_space")
        self.assertNotIn("ERROR:", stdout, "setup failed")
        self.assertNotIn("ERROR:", stderr, "setup failed")
        self.cleanup()
    ...
    def test_no_space__001_pos(self):
        lib.STFwrap.runScript(self, "/no_space/enospc_001_pos.py")
