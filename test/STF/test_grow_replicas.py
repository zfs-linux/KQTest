...
class test_grow_replicas(unittest.TestCase):
	...
	disk = self.d1[0].diskname
        ...
        (ret, stdout, stderr) = cmdQuery([path + "/grow_replicas/setup.py", disk, disk1], env=newenv, cwd=path+"/grow_replicas")
    ...
    def tearDown(self):
        (newenv, path) = lib.STFwrap.setupEnv()
        cmdQuery([path + "/grow_replicas/cleanup.py"], env=newenv, cwd=path+"/grow_replicas")
    ...
    def test_grow_replicas_001_pos(self):
        ...
        lib.STFwrap.runScriptArgs(self, ["/grow_replicas/grow_replicas_001_pos.py", disk2, disk3])
