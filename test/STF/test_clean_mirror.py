...
class test_clean_mirror(unittest.TestCase):
    def setUp(self):
        ...
        (ret, stdout, stderr) = cmdQuery([path + "/clean_mirror/setup.py" , disk0, disk1], env=newenv, cwd=path+"/clean_mirror")
    ...
    def cleanup(self):
        (newenv, path) = lib.STFwrap.setupEnv()
        cmdQuery([path + "/clean_mirror/cleanup.py"], env=newenv, cwd=path+"/clean_mirror")

    def tearDown(self):
        (newenv, path) = lib.STFwrap.setupEnv()
        (ret, stdout, stderr) = cmdQuery([path + "/clean_mirror/cleanup.py"], env=newenv, cwd=path+"/clean_mirror")
        self.assertNotIn("ERROR:", stdout, "setup failed")
        self.assertNotIn("ERROR:", stderr, "setup failed")
        self.cleanup()
	...
	disk = "/dev/" + disk
	lib.STFwrap.runScriptArgs(self, ["/clean_mirror/clean_mirror_001_pos.py", disk])

    def test_clean_mirror_002_pos(self):
	disk = self.d2[0].diskname
	disk = "/dev/" + disk
	lib.STFwrap.runScriptArgs(self, ["/clean_mirror/clean_mirror_002_pos.py", disk])

    def test_clean_mirror_003_pos(self):
	disk = self.d1[0].diskname
	disk = "/dev/" + disk
	lib.STFwrap.runScriptArgs(self, ["/clean_mirror/clean_mirror_003_pos.py", disk])

    def test_clean_mirror_004_pos(self):
	disk = self.d2[0].diskname
	disk = "/dev/" + disk
	lib.STFwrap.runScriptArgs(self, ["/clean_mirror/clean_mirror_004_pos.py", disk])
