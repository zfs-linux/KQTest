...
class test_large_files(unittest.TestCase):
    def setUp(self):
        ...
        (ret, stdout, stderr) = cmdQuery([path + "/large_files/setup.py" , disk], env=newenv, cwd=path+"/large_files")
    ...
    def cleanup(self):
        (newenv, path) = lib.STFwrap.setupEnv()
        cmdQuery([path + "/large_files/cleanup.py"], env=newenv, cwd=path+"/large_files")

    def tearDown(self):
        (newenv, path) = lib.STFwrap.setupEnv()
        (ret, stdout, stderr) = cmdQuery([path + "/large_files/cleanup.py"], env=newenv, cwd=path+"/large_files")
        self.assertNotIn("ERROR:", stdout, "setup failed")
        self.assertNotIn("ERROR:", stderr, "setup failed")
        self.cleanup()
    ...
    def test_large_files_001_pos(self):
        lib.STFwrap.runScript(self, "/large_files/large_files_001_pos.py")
